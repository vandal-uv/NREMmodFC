...
BOLD_output = '../../empirical/BOLD_empirical_AAL90'
...
for rx,entry in enumerate(entries):
    ...
    #LC signals
    LC_signals = np.load(BOLD_output+"/" +f'LC_bilateral_norm_nonfilt_S{entry}.npy')    
    ...
    ##this also includes the entries for the LC and BF
...
##note: this is not symmetrized, it is done later when analyzing
save_dic = {}
...
AAL_labels = pd.read_csv('../../empirical/ROI_MNI_V4.csv')['label'].values[0:90]
...
plt.tight_layout()
# plt.savefig("prefig1.svg",dpi=300)
