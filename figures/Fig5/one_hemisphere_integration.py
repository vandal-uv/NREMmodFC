...
emp_matts = {st:np.loadtxt(f"../../analyze_empirical/mean_mat_{st}_8dic24.txt") for st in states}
...
side2entries = {"all":range(90),
           "right":range(45,90),
           "left":range(45)}
side = "left"
entries = side2entries[side]
...
ax.set_title(f"Empirical nodal Integration {side}",fontsize=titlesais)
violin_plot(ax, [df_in[(df_in["state"]==st) & (df_in["mod"]=="emp")]["val"].values[entries] for st in states],
            color_names= colors,
            alpha_violin = 0.5)
...
ax.set_title(f"LC-FC vs Integration (Wake, {side})",y = 0.98,fontsize=titlesais)
x= FCmap_in[entries] ;y=Hin_node[entries]
a, b, r, p, ss = LR(x, y)
print(r,p)
...
x = FCmap_in[entries];y=Hse_node[entries]
a, b, r, p, ss = LR(x, y)
print(r,p)
